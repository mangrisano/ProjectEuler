...
def problem(low_limit, high_limit):
    abundants = create_abundant_list(low_limit, high_limit)
    sum_of_abundants = list()
    result = list()
    for i in abundants:
        for j in abundants:
            sum_of_abundants.append(i + j)

    result = (number for number in range(low_limit, high_limit) if number not in sum_of_abundants)
    return sum(result)


def is_abundant(number):
    return sum(factorize(number)) > number


def factorize(number):
    return (fact for fact in range(1, number) if not (number % fact))


def create_abundant_list(low_limit, high_limit):
